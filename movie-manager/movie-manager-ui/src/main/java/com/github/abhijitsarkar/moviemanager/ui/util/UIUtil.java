...
public class UIUtil {
    ...
    private static URL SUCCESS_SOUND = UIUtil.class.getResource(
            Sound.SUCCESS.toString());
    private static URL ERROR_SOUND = UIUtil.class.getResource(
            Sound.ERROR.toString());
    ...
    public static void cleanup(boolean success, JButton button,
            JLabel label, String message,
            String successResult, Exception... onlyWhenError) {
...
//            }
            ...
            logger.error("Failed to play the audio file: " + sound.toString(), ex);
        } finally {
            clip.stop();
            clip.close();
            try {
                ...
                logger.error("Failed while attempting to close the audio stream.", ex);
            }
        }
    }
